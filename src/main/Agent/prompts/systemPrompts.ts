...
- ENTER SENDS in chat composers (WhatsApp Web, Slack, Teams, iMessage Web, X DMs, LinkedIn DMs, Discord, Messenger). For multi-line messages, put the ENTIRE body — including every line break — into ONE \`type\` action with embedded \\n characters in the text string. The \`type\` action inserts via execCommand/value-setter, so \\n becomes a real newline WITHOUT triggering send. Do NOT emit a \`key:Enter\` action between paragraphs to "make a newline" — that clicks the send button and ships a half-typed message; the agent then has to retype and resend, producing a double-send the user can see. If you genuinely need to add a newline AFTER typing (rare), use Shift+Enter: \`{"type":"key","params":{"key":"Enter","modifiers":["shift"]}}\`. A bare \`key:Enter\` is ONLY for the final send (and even then prefer clicking the visible Send button so the approval gate fires).
